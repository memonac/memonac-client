...
- 초대한 사람의 인증 방법
  ...
  먼저 초대한 사람인지 아닌지 인증하는 방법으로 query string parameter에 토큰을 담은 링크를 초대 받은 사람에게 메일로 보내기로 했습니다.

- 메모룸의 참여자로 추가

  메일 발송 직후 바로 해당 메모룸의 참여자로 추가하고 토큰만 인증한 후 바로 해당 메모룸으로 가게 하는 방법을 생각했었는데 이 방법은 만약 그 사람이 **링크를 클릭하지 않았을 경우가 문제**가 되기 때문에 채택되지 않았습니다. 그래서 무조건 링크가 클릭 되고, **토큰이 인증이 되었을때 메모룸의 참여자로 추가**가 되어야 한다고 팀원들과 의견을 모았습니다.

- 메모룸으로 바로 이동 vs 로그인 후 메모룸 이동

  해당 링크를 클릭하면 인증하는 로직을 거쳐 해당 메모룸으로 바로 이동하는 경우 **토큰을 담은 링크가 탈취**된다면 큰 문제가 생길 수 있기 때문에 메모룸을 바로 이동하는 것은 위험하다고 생각했습니다. 그래서 로그인 과정을 한번 더 거치고 **로그인이 완료되면 메모룸 리스트에 초대받은 메모룸이 추가되는 방식**을 선택하게 되었습니다.

- 추가적으로 고려해야할 점

  위와 같은 로직은 만약 해당 링크를 클릭만하고 로그인을 하지 않을 경우에도 메모룸 리스트에 추가되기 때문에 조금 더 세세한 인증방법이 필요하다고 생각됩니다. 그래서 이 문제를 해결하기 위해 어떤 상태값을 주고 로그인까지 완료되었을때 상태값을 변경하여 초대된 메모룸을 추가해주는 로직으로 발전시킨다면 보완할 수 있을거라 생각합니다.
<br>
<br>
<br>
